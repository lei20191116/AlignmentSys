...
def get_users_posts(user,env, headless=True, wait=2,limit=float("inf")):
    driver = utils.init_driver(headless=headless)
    users_posts(user,driver)


    tweet_ids = set()

    posts = utils.get_users_posts(headless=True, env=env,user=user,scrolling=True,wait=2)

def users_posts(user,driver):
    sleep(random.uniform(1, 2))
    driver.get('https://twitter.com/' + user)
    sleep(random.uniform(1, 2))
